(() => {
  ...
  function validateEnv() {
    if (!window.CARITAS?.supabase || !window.CARITAS?.auth || !window.QRCode) {
  ...
  if (btnCrear) {
    btnCrear.addEventListener("click", async () => {
      ...
      try {
        ...
        // 📌 Subir foto a Supabase Storage (bucket correcto)
        const ext = foto.name.split(".").pop();
        const remotePath = `${user.uid}/${Date.now()}_${Math.random()
          .toString(36)
          .substring(2)}.${ext}`;

        const { error: uploadError } = await supabase.storage
          .from("credenciales-fotos")
          .upload(remotePath, foto, { upsert: true });
